...
**v0.3.4** (2026-07-31, built from main 955424db) - a linked Git worktree now inherits the allowlist consent of its main repository, so sessions started in a fresh worktree no longer need a separate scope confirmation.
...
1. Скачай [`desktop/fractal-mcp-0.3.4.mcpb`](desktop/fractal-mcp-0.3.4.mcpb).
...
cd desktop && npx @anthropic-ai/mcpb pack . fractal-mcp-0.3.4.mcpb
```
